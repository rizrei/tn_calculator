defmodule CalculatorTest do
  ...
  describe "add/2" do
    ...
    test "when n1 float and n2 float and result integer" do
      assert Calculator.add(1.1, 2.9) == {:ok, 4}
    end

    test "when atom argument" do
      assert Calculator.add(:not_a_number, 2.1) ==
               {:error, "Invalid argument! :not_a_number must be a number"}
    end

    test "when tuple argument" do
      assert Calculator.add({1}, 2.1) ==
               {:error, "Invalid argument! {1} must be a number"}
    end

    test "when list argument" do
      assert Calculator.add([1], 2.1) ==
               {:error, "Invalid argument! [1] must be a number"}
    end
  end
  ...
  describe "sub/2" do
    ...
    test "when n1 float and n2 float" do
      assert Calculator.sub(1.1, 2.1) == {:ok, -1}
    end

    test "when atom argument" do
      assert Calculator.sub(:not_a_number, 2.1) ==
               {:error, "Invalid argument! :not_a_number must be a number"}
    end
  ...
  describe "mult/2" do
    ...
    test "when atom argument" do
      assert Calculator.mult(:not_a_number, 2.1) ==
               {:error, "Invalid argument! :not_a_number must be a number"}
    end

    test "when tuple argument" do
      assert Calculator.mult({1}, 2.1) ==
               {:error, "Invalid argument! {1} must be a number"}
    end
  ...
  describe "div/2" do
    test "when n1 integer and and n2 integer" do
      assert Calculator.div(6, 2) == {:ok, 3}
    end

    test "when n1 float and and n2 integer" do
      assert Calculator.div(6.0, 2) == {:ok, 3}
    end

    test "when n1 integer and n2 float" do
      assert Calculator.div(6, 1.5) == {:ok, 4}
    end
    ...
    test "when n1 float and n2 float" do
      assert Calculator.div(6.0, 1.5) == {:ok, 4}
    end
    ...
    test "when atom argument" do
      assert Calculator.div(:not_a_number, 2.1) ==
               {:error, "Invalid argument! :not_a_number must be a number"}
    end

    test "when tuple argument" do
      assert Calculator.div({1}, 2.1) ==
               {:error, "Invalid argument! {1} must be a number"}
    end
